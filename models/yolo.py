...
from utils.plots import feature_visualization, feature_visualization_merged
...
class Segment_KSP(Detect):
    ...
    def __init__(self, nc=80, anchors=(), nm=32, npr=256, ch=(), inplace=True):
        ...
        self.m = nn.ModuleList([
            nn.Conv2d(ch[0], self.no * self.na, 1),  # P3, use 1x1 conv
            nn.Conv2d(ch[1], self.no * self.na, 2, padding=1),  # P4, use 2x2 conv
            nn.Conv2d(ch[2], self.no * self.na, 3, padding=1),  # P5, use 3x3 conv
        ])
        self.proto = Proto(ch[0], self.npr, self.nm)  # protos
        self.detect = Detect.forward
...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            # this will forward all moudel in cpu, if can't run on cpu, need to move to gpu
            if m.__class__.__name__ in {'H_SS2D', 'DFB', 'H_VSS', 'FSSM'}:
            ...
            if visualize:
                ...
                feature_visualization_merged(x, m.type, m.i, save_dir=visualize, merge_mode="max")
        return x
            ...
            try:
                ...
                if 'Not implemented on the CPU' in str(
                    ...
                    m.stride = torch.tensor([s / x.shape[-2]
                                             for x in forward(torch.zeros(2, ch, s, s).to('cuda'))])  # forward
                else:
                    raise e
            check_anchor_order(m)
            # let tow value to same device
            # flag original device
            device = m.anchors.device if m.anchors.is_cuda else 'cpu'
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m is C3k2:  # for M/L/X sizes cause our use l scale, it usually use k2 scale
                args[3] = True
