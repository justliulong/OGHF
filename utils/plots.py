...
class Annotator:
    # YOLO Annotator for train/val mosaics and jpgs and detect/hub inference annotations
    ...
    def masks_instance(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        """
        Plot masks on image.
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):
    ...
    if 'Detect' not in module_type:
        # x 必须可以通过shape获取到batch, channels, height, width，否则直接return
        if not hasattr(x, 'shape'):
            print('feature_visualization(): x must have shape')
            return
