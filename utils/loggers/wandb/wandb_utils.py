...
ROOT = FILE.parents[3]  # YOLO root directory
...
class WandbLogger():
       ...
       """
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            ...
            self.wandb_run = wandb.init(config=opt,
                                        resume="allow",
                                        project='YOLO' if opt.project == 'runs/train' else Path(opt.project).stem,
                                        entity=opt.entity,
    ...
    def check_and_upload_dataset(self, opt):
        ...
        config_path = self.log_dataset_artifact(opt.data, opt.single_cls,
                                                'YOLO' if opt.project == 'runs/train' else Path(opt.project).stem)
