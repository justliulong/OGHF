...
🔨 Track every YOLO training run in the <b>experiment manager</b>

🔧 Version and easily access your custom training data with the integrated ClearML <b>Data Versioning Tool</b>

🔦 <b>Remotely train and monitor</b> your YOLO training runs using ClearML Agent

🔬 Get the very best mAP using ClearML <b>Hyperparameter Optimization</b>

🔭 Turn your newly trained <b>YOLO model into an API</b> with just a few commands using ClearML Serving
...
This will enable integration with the YOLO training script. Every training run from now on, will be captured and stored by the ClearML experiment manager.

If you want to change the `project_name` or `task_name`, use the `--project` and `--name` arguments of the `train.py` script, by default the project will be called `YOLO` and the task `Training`.
...
The YOLO repository supports a number of different datasets by using yaml files containing their information. By default datasets are downloaded to the `../datasets` folder in relation to the repository root folder. So if you downloaded the `coco128` dataset using the link in the yaml or with the scripts provided by yolov5, you get this folder structure:
...
```bash
cd coco128
clearml-data sync --project YOLO --name coco128 --folder .
```
...
clearml-data create --name coco128 --project YOLO
...
Now that you have a ClearML dataset, you can very simply use it to train custom YOLO 🚀 models!
...
To run the YOLO training script remotely, all you have to do is add this line to the training.py script after the clearml logger has been instatiated:
```python
# ...
# Loggers
data_dict = None
