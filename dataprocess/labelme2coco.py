''' 
直接将labelme数据集转为coco数据集，并划分训练集和验证集
命令行执行：  python labelme2coco.py --input_dir images --output_dir coco --labels labels.txt
输出文件夹必须为空文件夹

source: https://github.com/wkentaro/labelme/blob/main/examples/instance_segmentation/labelme2coco.py
    ...
    for image_id, filename in enumerate(label_files):
        ...
        data["images"].append(
            dict(
                ...
                height=img.shape[0],
                width=img.shape[1],
                date_captured=time.strftime("%Y-%m-%d %H:%M:%S.%f"),
                id=image_id,
            ))
        ...
        # ************************** 可视化的处理开始 *******************************************
        if not args.noviz:
            labels, captions, masks = zip(*[(class_name_to_id[cnm], cnm, msk) for (cnm, gid), msk in masks.items()
            ...
            out_viz_file = osp.join(args.o, "visualization", base + ".jpg")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--i", help="input annotated directory")
    parser.add_argument("--o", help="output dataset directory")
    parser.add_argument("--l", help="labels file", required=True)
    parser.add_argument("--noviz", help="no visualization", action="store_true")
    args = parser.parse_args()

    if osp.exists(args.o):
        print("Output directory already exists:", args.o)
        sys.exit(1)
    os.makedirs(args.o)
    print("| Creating dataset dir:", args.o)
    if not args.noviz:
        os.makedirs(osp.join(args.o, "visualization"))
